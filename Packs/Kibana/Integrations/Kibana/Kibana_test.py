...
def test_context_creation_es7(params, mocker):
    ...
    importlib.reload(Kibana)  # To reset the Elasticsearch client with the OpenSearch library
    from Kibana import get_total_results, results_to_context
...
def test_incident_creation_e6(params, mocker):
    mocker.patch.object(demisto, "params", return_value=params)
    importlib.reload(Kibana)  # To reset the Elasticsearch client with the OpenSearch library
    from Kibana import results_to_incidents_datetime
...
def test_timestamp_to_date_converter_seconds(params, mocker):
    mocker.patch.object(demisto, "params", return_value=params)
    importlib.reload(Kibana)  # To reset the Elasticsearch client with the OpenSearch library
    mocker.patch("Kibana.TIME_METHOD", "Timestamp-Seconds")
    from Kibana import timestamp_to_date
...
def test_timestamp_to_date_converter_milliseconds(params, mocker):
    mocker.patch.object(demisto, "params", return_value=params)
    importlib.reload(Kibana)  # To reset the Elasticsearch client with the OpenSearch library
    mocker.patch("Kibana.TIME_METHOD", "Timestamp-Milliseconds")
    from Kibana import timestamp_to_date
...
def test_incident_creation_with_timestamp_e7(params, mocker):
    ...
    importlib.reload(Kibana)  # To reset the Elasticsearch client with the OpenSearch library
    mocker.patch("Kibana.TIME_METHOD", "Timestamp-Seconds")
    from Kibana import results_to_incidents_timestamp
...
def test_format_to_iso(params, mocker):
    mocker.patch.object(demisto, "params", return_value=params)
    importlib.reload(Kibana)  # To reset the Elasticsearch client with the OpenSearch library
    from Kibana import format_to_iso
...
def test_elasticsearch_builder_called_with_username_password(params, mocker):
    mocker.patch.object(demisto, "params", return_value=params)
    importlib.reload(Kibana)  # To reset the Elasticsearch client with the OpenSearch library
    from Kibana import Elasticsearch, elasticsearch_builder
...
def test_elasticsearch_builder_called_with_no_creds(params, mocker):
    mocker.patch.object(demisto, "params", return_value=params)
    importlib.reload(Kibana)  # To reset the Elasticsearch client with the OpenSearch library
    mocker.patch("Kibana.USERNAME", None)
    mocker.patch("Kibana.PASSWORD", None)
    from Kibana import Elasticsearch, elasticsearch_builder
...
def test_elasticsearch_parse_subtree(params, mocker):
    mocker.patch.object(demisto, "params", return_value=params)
    importlib.reload(Kibana)  # To reset the Elasticsearch client with the OpenSearch library
    from Kibana import parse_subtree
...
@patch("Kibana.FETCH_INDEX", "customer")
...
class TestIncidentLabelMaker(unittest.TestCase):
    def test_sanity(self):
        from Kibana import incident_label_maker
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "",
            "",
            {"range": {"time_field": {"gt": "2.1.2000 12:00:00.000000", "format": Kibana.ES_DEFAULT_DATETIME_FORMAT}}},
        ),
    ],
)
def test_get_time_range(time_method, last_fetch, time_range_start, time_range_end, result):
    Kibana.TIME_METHOD = time_method
    from Kibana import get_time_range
...
def test_build_eql_body():
    from Kibana import build_eql_body
...
def test_get_mapping_fields_command(mocker, indexes, response, expected_result):
    ...
    mocker.patch("Kibana.FETCH_INDEX", indexes)
    mocker.patch("Kibana.requests.get", return_value=ResponseMockObject())
    result = Kibana.get_mapping_fields_command()
...
def test_search_command_with_query_dsl(mocker):
    ...
    Kibana.FETCH_INDEX = "index from parameter"
    ...
    search_mock = mocker.patch.object(Kibana.Elasticsearch, "search", return_value=ES_V7_RESPONSE)
    mocker.patch.object(Kibana.Elasticsearch, "__init__", return_value=None)
    Kibana.search_command({})
...
def test_execute_raw_query(mocker, raw_query_body):
    ...
    Kibana.FETCH_INDEX = "index from parameter"
    mocker.patch.object(Kibana.Elasticsearch, "search", return_value=ES_V7_RESPONSE)
    mocker.patch.object(Kibana.Elasticsearch, "__init__", return_value=None)
    es = Kibana.elasticsearch_builder({})
    assert Kibana.execute_raw_query(es, json.dumps(raw_query_body)) == ES_V7_RESPONSE
...
def test_execute_raw_query_v8(mocker, raw_query_body):
    ...
    Kibana.RequestsHttpNode = RequestsHttpNode

    class CustomExecute:
        def to_dict():  # type: ignore
            return ES_V8_RESPONSE

    mocker.patch.object(Kibana, "ELASTIC_SEARCH_CLIENT", Kibana.ELASTICSEARCH_V8)
    mocker.patch.object(Kibana.Elasticsearch, "search", return_value=ES_V7_RESPONSE)
    mocker.patch.object(Kibana.Search, "execute", return_value=CustomExecute)
    mocker.patch.object(Kibana.Elasticsearch, "__init__", return_value=None)
    mocker.patch.object(RequestsHttpNode, "__init__", return_value=None)

    es = Kibana.elasticsearch_builder({})
    assert Kibana.execute_raw_query(es, json.dumps(raw_query_body)) == ES_V8_RESPONSE
...
def test_convert_date_to_timestamp(date_time, time_method, expected_time):
    ...
    """
    Kibana.TIME_METHOD = time_method
    assert Kibana.convert_date_to_timestamp(date_time) == expected_time
...
def test_index_document(mocker):
    ...
    mocker.patch.object(Kibana.Elasticsearch, "index", return_value=MOCK_INDEX_RESPONSE)
    mocker.patch.object(Kibana.Elasticsearch, "__init__", return_value=None)
    assert Kibana.index_document({"index_name": "test-index", "document": "{}", "id": "1"}, "") == MOCK_INDEX_RESPONSE
...
def test_index_document_command(mocker):
    ...
    mocker.patch.object(Kibana.Elasticsearch, "index", return_value=MOCK_INDEX_RESPONSE)
    mocker.patch.object(Kibana.Elasticsearch, "__init__", return_value=None)
    command_result = Kibana.index_document_command({"index_name": "test-index", "document": "{}", "id": "1"}, "")
...
def test_get_value_by_dot_notation():
    ...
    result = Kibana.get_value_by_dot_notation(dictionary, key)
...
def test_get_indices_statistics_command(mocker, limit, all_results, expected_context):
    ...
    mocker.patch.object(Kibana, "get_indices_statistics", return_value=MOCK_INDICES_STATISTICS_RESPONSE)
    mocker.patch.object(Kibana.Elasticsearch, "__init__", return_value=None)
    command_result = Kibana.get_indices_statistics_command({"limit": limit, "all_results": all_results}, "")
...
def test_verify_es_server_version_errors(mocker, server_details, server_version, client_version):
    ...
    mocker.patch("Kibana.ELASTIC_SEARCH_CLIENT", new=client_version)
    with pytest.raises(ValueError) as e:
        Kibana.verify_es_server_version(server_details)
...
def test_search_command_with_query_esql(mocker):
    ...
    mocked_builder = mocker.patch.object(Kibana, "elasticsearch_builder")
    mocked_builder().perform_request.return_value = magic_mock
    mocker.patch.object(Kibana, "ELASTIC_SEARCH_CLIENT", new=Kibana.ELASTICSEARCH_V9)

    query = """FROM alerts | WHERE alertDetails.alertuser LIKE "*karl*"| KEEP *"""
    res = Kibana.search_esql_command({"query": query, "limit": 2}, {})
